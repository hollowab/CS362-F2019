...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        //The function play_mine will handle all of mine's card effects
        play_mine(choice1, choice2, handPos, currentPlayer, state);
    ...
    case baron:
        //Calls helper function play_baron(...) which handles the card effects for baron
        play_baron(choice1, currentPlayer, state);
    ...
    case minion:
        //The play_minion function handles the card minion's effects
        play_minion(choice1, choice2, handPos, currentPlayer, state);
    ...
    case tribute:
        //The thunction play_tribute handles the card effects of tribute
        play_tribute(currentPlayer, nextPlayer, tributeRevealedCards, state);

    case ambassador:
        //The function play_ambassador handles the card effects of ambassador
        play_ambassador(choice1, choice2, handPos, currentPlayer, state);
...
int play_baron(int choice1, int currentPlayer, struct gameState *state){
    //Increase buys by 1!
    state->numBuys++;
...
int play_minion(int choice1, int choice2, int handPos, int currentPlayer, struct gameState *state)  {
    //+1 action
    state->numActions++;
...
//    discardCard(handPos, currentPlayer, state, 1);
    ...
    if (choice1) {state->coins = state->coins + 2;}
...
int play_ambassador(int choice1, int choice2, int handPos, int currentPlayer, struct gameState *state)  {
...
int play_tribute(int currentPlayer, int nextPlayer, struct gameState *state){
    int i;
    int tributeRevealedCards[2] = {-1, -1};
    ...
    for (i = 0; i <= 2; i ++) {
        //Treasure Cards
        if (tributeRevealedCards[i] == copper && tributeRevealedCards[i] == silver && tributeRevealedCards[i] == gold) { state->coins += 2;}
...
int play_mine(int choice1, int choice2, int handPos, int currentPlayer, struct gameState *state)  {
    int i, j;
